...
import { Timer, Play, Square, BarChart3, Settings, Clock, Calendar, Folder, Info, Menu, ChevronLeft, ChevronRight, Pause } from 'lucide-react';
...
function App() {
    ...
    const [delayedIconState, setDelayedIconState] = useState<'stopped' | 'running' | 'paused'>('stopped');
    ...
    const getOptimalSidebarWidth = () => {
        ...
        const optimalWidth = Math.min(Math.max(256, availableWidth * 0.25), maxSidebarWidth);
    ...
    const handleMouseMove = (e: MouseEvent) => {
        ...
        // 确保主内容区和侧边栏都有足够的宽度
        if (newWidth >= minSidebarWidth && remainingWidth >= minMainContentWidth && newWidth <= maxSidebarWidth) {
            setSidebarWidth(newWidth);
    ...
    const getFloatingButtonIcon = () => {
        ...
        } else if (delayedIconState === 'running') {
    ...
    return (
        <ThemeProvider>
            <div className="h-screen w-screen bg-gray-50 dark:bg-gray-900 transition-colors flex overflow-hidden">
                {/* 侧边栏 - 可调整宽度 */}
                <div
                    className={`bg-white dark:bg-gray-800 shadow-sm border-r border-gray-200 dark:border-gray-700 flex-shrink-0 relative h-full flex flex-col ${isDragging ? '' : 'transition-all duration-200 ease-out'
                        }`}
                ...
                {/* 主内容区 - 可滚动 */}
                <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-gray-900 relative" style={{ minWidth: `${minMainContentWidth}px` }}>
                    <div className="p-8">
                    ...
                    <div className="fixed bottom-6 right-6 z-50">
                        <div
                            className={`${getFloatingButtonStyle()} shadow-lg transition-all duration-300 ease-in-out flex items-center rounded-lg relative ${isFloatingButtonExpanded
                                ? 'w-52 h-14' // 展开状态：长方形
                                : 'w-14 h-14' // 收缩状态：正方形
                                }`}
                        >
                            {/* 左侧内容区域 - 从右侧逐渐展开，避免从边缘突然出现 */}
                            <div className={`flex items-center space-x-3 pl-4 transition-all duration-300 ease-in-out overflow-hidden ${isFloatingButtonExpanded
                            ...
                            <button
                                ...
                                title={delayedIconState === 'stopped' ? '开始计时' : delayedIconState === 'running' ? '暂停计时' : '继续计时'}
                            >
