...
import type { RouteId, RouteState, RouterConfig, RouterContext, RouterActions, NavigationSource, RouteRecord } from '../types/router';
...
export function useRouterState(initialConfig?: Partial<RouterConfig>) {
  ...
  // 保存状态到localStorage
  const saveState = useCallback((newState: RouteState) => {
    if (config.rememberNavigation) {
  ...
  // 更新配置
  const updateConfig = useCallback((newConfig: Partial<RouterConfig>) => {
  ...
  // 导航到指定路由
  const navigate = useCallback((route: RouteId, source: NavigationSource = 'direct') => {
    setState(prevState => {
